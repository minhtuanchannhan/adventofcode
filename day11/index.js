...
const compute = (part2 = false, rounds) => {
  ...
  const op = (old, pars) =>
    pars[1] == "+" ? val(pars[0], old) + val(pars[2], old) : val(pars[0], old) * val(pars[2], old);
  const round = () =>
    monkeys.forEach((monkey) => {
      while (monkey.stack.length) {
        let item = monkey.stack.shift();
        item = part2 ? op(item, monkey.opPars) % mod : Math.floor((op(item, monkey.opPars) % mod) / 3);
